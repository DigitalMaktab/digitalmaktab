...
builder.Services
.AddControllers()
// Controllers live in DigitalMaktab.Core — register that assembly so ASP.NET Core
// discovers them (default discovery only scans the entry-point assembly).
.AddApplicationPart(typeof(digitalmaktabapi.Controllers.BaseController).Assembly)
...
// Add Services
// Mapper profiles (BaseMapper, SchoolMapper, StudentMapper, TeacherMapper, HeaderMapper, AppMapper)
// live in DigitalMaktab.Core — scan that assembly for AutoMapper Profile subclasses.
builder.Services.AddAutoMapper(typeof(digitalmaktabapi.Helpers.Mappers.BaseMapper).Assembly);
...
// Resources folder lives in DigitalMaktab.Core and is copied to the API's output
// via Core.csproj's <Content CopyToOutputDirectory>. AppContext.BaseDirectory
// resolves to the bin folder in dev and the publish folder in prod.
...
else
{
    app.UseStaticFiles(new StaticFileOptions
    {
        FileProvider = new CompositeFileProvider(
            new PhysicalFileProvider(Path.Combine(app.Environment.ContentRootPath, "wwwroot")),
            new PhysicalFileProvider(resourcesPath)
        ),
        RequestPath = ""
    });
}
