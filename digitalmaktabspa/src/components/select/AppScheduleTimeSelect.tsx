...
const AppScheduleTimeSelect: React.FC<SelectProps> = ({
  value,
  onChange,
  name,
}) => {
  const { t } = useTranslation();
  const { fetchScheduleTimes, data } = useMainOperations();
  const [options, setOptions] = useState<Select2Option[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      await fetchScheduleTimes(1, 10, {});
    };
...
export default AppScheduleTimeSelect;
